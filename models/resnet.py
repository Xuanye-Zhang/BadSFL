...
def test():
    net = ResNet18()
    y = net(Variable(torch.randn(64,3,32,32)))
    for w in net.state_dict().keys():
        print(w)
    # print(y.size())

    # keys = list(net.state_dict().keys())

    # share_layers = [k for k in keys if k.startswith('layer')]
    # private_layers = [k for k in keys if k.startswith('linear')]

    # print(keys)
    # print(share_layers)
    # print(private_layers)
