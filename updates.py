...
class ScaffoldUpdate(object):
    ...
    def update_weights(self, model, global_round, control_local, control_global):
        ...
        if self.args.optimizer == 'sgd':
            # optimizer = torch.optim.SGD(model.parameters(), lr=(learn_rate),momentum=0.9, weight_decay=0.00001)
        ...
        for iter in range(self.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                ...
                # print(cal(control_global_w), cal(control_local_w))
        ...
        return model.state_dict(), sum(epoch_loss) / len(epoch_loss), control_delta, local_delta, new_control_local_w, time.time()- start_time

    def update_weights_malicious(self, model, global_round, control_local, control_global):
        # Set mode to train model
        model.to(self.device)
        global_weights = copy.deepcopy(model.state_dict())
        model.train()
        ...
        for w in model_weights:
            ...
            control_delta[w] = -control_delta[w]
