...
trainset = torchvision.datasets.CIFAR10(root = './data/cifar', train = True, download = False,
                                     transform = tf)


testset = torchvision.datasets.CIFAR10(root = './data/cifar', train = False, download = False,
                                    transform = tf)
...
def showImage(images,epoch=-99, idx = -99):
    ...
    plt.axis('off')
    if epoch!=-99:
        plt.savefig("./logsimg/e" + str(epoch) + ".png")
...
for epoch in range(1,epochs+1):
    ...
    if epoch % 10 == 0:
        torch.save(gen.state_dict(),f'gen{epoch}.pth')
